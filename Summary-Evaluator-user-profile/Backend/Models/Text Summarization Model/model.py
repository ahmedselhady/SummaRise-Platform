...
class SummarizationModel(object):
  ...
  def __init__(self, hps, vocab):
    self._hps = hps
    self._vocab = vocab
    self.pointer_gen = hps.pointer_gen
    self.coverage = hps.coverage
  ...
  #   """Setup so that we can view word embedding visualization in Tensorboard:

  #   Make the vocab metadata file, then make the projector config file pointing to it."""
  #   train_dir = os.path.join(self.log_root, "train")
  ...
  def _add_seq2seq(self):
    ...
    with tf.variable_scope('seq2seq'):
      ...
      with tf.variable_scope('embedding'):
        ...
        if hps.mode=="train": pass #self._add_emb_vis(embedding) # add to tensorboard
